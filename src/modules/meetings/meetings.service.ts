...
export class MeetingsService {
  ...
  getByTeacher(teacherId: string): any[] {
    return db.prepare('SELECT * FROM meetings WHERE teacher_id = ? ORDER BY date ASC').all(teacherId)
  }

  getNext(): any {
    const now = new Date().toISOString()
    return db.prepare("SELECT * FROM meetings WHERE date > ? ORDER BY date ASC LIMIT 1").get(now) || null
  }
